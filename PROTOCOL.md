...
(generated by `scripts/build-fixtures.js` — every report byte, including
padding and checksums, is computed, never hand-transcribed). Capture tooling:
`scripts/capture-hook.js`, `scripts/verify-checksums.js`,
`scripts/vendor-source-excerpt.js`, `scripts/check-coverage.js` (the `bin/ci`
gate).
