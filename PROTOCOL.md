...
Machine-readable source of truth: `fields.json`. Raw evidence: `fixtures/`.
Capture tooling: `scripts/capture-hook.js`, `scripts/verify-checksums.js`,
`scripts/vendor-source-excerpt.js`.
...
Every 64-byte report:

```
offset  0      : opcode (0x40 / 0x41 / 0x42)
offset  1-2    : 0x00 0x00 (reserved)
offset  3      : length (of the payload that follows)
offset  4(-5)  : checksum -- 1 byte for 0x41/0x42, 2 bytes little-endian for 0x40
offset  5 or 6 : 0x00 (reserved)
...
type (output report), report ID (0), interface identity, opcode table, outer
report structure, both checksum variants, and the full clock+date payload
layout.
