...
const PAIRS = [
  ...
  // Milestone 4. Weaker than the picture pair by construction: the fixture's
  // pixel bytes are copied from this same capture (the vendor's browser
  // resampling is not reproducible), so those bytes cannot disagree. What this
  // pair genuinely tests is everything AROUND them -- the control reports, the
  // 1024-byte block boundaries, the per-block offset restart, the length bytes
  // and every checksum, all regenerated by the model.
  { rawFile: 'raw/cap-gif-upload-hidlog.json', fixtureFile: 'gif-upload.json' },
  // Milestone 7. Real captured ACKs (not withAckFlip-derived), unlike the
  // page-switch/clear-picture raw files above -- so this pair also checks
  // that the flip assumption those two rely on actually held here.
  { rawFile: 'raw/cap-clear-gif-hidlog.json', fixtureFile: 'clear-gif.json' },
];
